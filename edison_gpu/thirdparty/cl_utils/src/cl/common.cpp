...
bool cl::initOpenCL() {
    int clew_init_code = clewInit();
    if (clew_init_code != 0) {
        verbose_cerr << "OpenCL init failed with code " << clew_init_code << "!" << std::endl;
        return false;
    } else {
        return true;
    }
...
std::string cl::getErrorString(cl_int errorCode) {
    // From http://stackoverflow.com/a/24336429/1549330
    switch (errorCode) {
...
bool cl::Version::operator<(Version other) const  {
    return (majorVersion < other.majorVersion || (majorVersion == other.majorVersion && minorVersion < other.minorVersion));
}
